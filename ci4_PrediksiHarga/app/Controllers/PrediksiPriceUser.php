...
class PrediksiPriceUser extends BaseController
{
    public function index()
    {
        return view('prediksi_viewuser');
    }
    ...
    public function proses_prediksiharga()
{
    ...
    $output = shell_exec("python $python_script $grade_mold $qty_produk $tonase $cavity_material_encoded $core_material_encoded $slide_system_encoded $lift_core_system_encoded 2>&1");
    ...
    if ($output !== null) {
        ...
        $fp = fopen($filePath, 'w');
        if ($fp === false) {
            return view('prediksi_resultuser', ['error_message' => 'Gagal membuat file untuk menyimpan prediksi.']);
        }
        foreach ($predictions as $prediction) {
        ...
        return view('prediksi_resultuser', [
        ...
        return view('prediksi_resultuser', ['error_message' => 'Gagal menjalankan script Python.']);
    }
}
    ...
    public function exportToExcel()
    {
        ...
        $drawing->setHeight(150);
        $drawing->setCoordinates('F25'); // Koordinat gambar dalam spreadsheet
        $drawing->setWorksheet($sheet);
        ...
        $sheet->setCellValue('A40', '5. Cavity Material :');
        $sheet->setCellValue('D40', $cavity_material);
        $sheet->setCellValue('A41', '6. Core Material :');
        $sheet->setCellValue('D41', $core_material);
        $sheet->setCellValue('A42', '7. Slide System :');
        $sheet->setCellValue('D42', $slide_system);
        $sheet->setCellValue('A43', '8. Lift Core System :');
        $sheet->setCellValue('D43', $lift_core_system);
    ...
    public function listUploadedFiles()
{
    ...
    return view('list_filesuser', ['fileList' => $fileList]);
}
