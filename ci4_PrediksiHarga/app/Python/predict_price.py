...
def predict_price_and_remark(grade_mold, part_application, qty_produk, tonase, cavity_material, core_material, slide_system, lift_core_system):
    ...
    X = data[['grade-mold', 'part-application', 'qty-produk', 'Tonase', 'Cavity-Material', 'Core-Material', 'Slide-System', 'Lift-Core-System']]
    y_remark = data['remark']
    y_price = data['price-mold']

    x_train, x_test, y_train_remark, y_test_remark = train_test_split(X, y_remark, test_size=0.1, random_state=0)
    _, _, y_train_price, y_test_price = train_test_split(X, y_price, test_size=0.1, random_state=0)
    ...
    input_data = pd.DataFrame([[grade_mold, part_application,  qty_produk, tonase, cavity_material, core_material, slide_system, lift_core_system]], columns=['grade-mold', 'part-application','qty-produk', 'Tonase', 'Cavity-Material', 'Core-Material', 'Slide-System', 'Lift-Core-System'])
...
def load_data_from_mysql(datasetmold):
    ...
    try:
        ...
        if connection.is_connected():
            ...
            cursor.execute(f"SELECT `grade-mold`, `part-application`, `qty-produk`, `Tonase`, `Cavity-Material`, `Core-Material`, `Slide-System`, `Lift-Core-System`, `remark`, `price-mold` FROM {datasetmold}")
            records = cursor.fetchall()
            df = pd.DataFrame(records, columns=['grade-mold', 'part-application','qty-produk', 'Tonase', 'Cavity-Material', 'Core-Material', 'Slide-System', 'Lift-Core-System', 'remark', 'price-mold'])

            label_encoder = LabelEncoder()
            for col in ['grade-mold', 'part-application', 'Cavity-Material', 'Core-Material', 'Slide-System', 'Lift-Core-System']:
...
if __name__ == "__main__":
    if len(sys.argv) < 9:
        print("Usage: python predict_price.py <grade_mold> <part_application> <qty_produk> <tonase> <cavity_material> <core_material> <slide_system> <lift_core_system>")
        sys.exit(1)
    ...
    cavity_material = sys.argv[5]
    core_material = sys.argv[6]
    slide_system = sys.argv[7]
    lift_core_system = sys.argv[8]

    prediction = predict_price_and_remark(grade_mold, part_application, qty_produk, tonase, cavity_material, core_material, slide_system, lift_core_system)
