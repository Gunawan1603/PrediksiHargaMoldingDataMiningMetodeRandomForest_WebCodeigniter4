...
<div class="text-left">
    ...
    <a href="<?= base_url('splitdatasetuser/showResults'); ?>" class="btn btn-primary">Result View</a>
...
<script>
    document.getElementById('btn-reset').addEventListener('click', function () {
        ...
        fetch('<?= base_url("splitdatasetuser/resetStatus"); ?>')
            .then(response => response.json())
            .then(data => {
                $('#loadingModal').modal('show'); // Hide the loading modal
                if (data.status === 'success') {
                    ...
                    fetch('<?= base_url("splitdatasetuser/deleteAllData"); ?>')
                        .then(response => response.json())
                        .then(data => {
                            if (data.status === 'success') {
    ...
    document.getElementById('btn-proses').addEventListener('click', function () {
        ...
        if (percentage !== '') {
            $('#loadingModal').modal('show'); // Show the loading modal
            fetch('<?= base_url("splitdatasetuser/splitPersen"); ?>', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    fetch('<?= base_url("splitdatasetuser/performance"); ?>', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
                    ...
                    .then(data => {
                        ...
                        if (data.status === 'success') {
                            ...
                            window.location.href = '<?= base_url("splitdatasetuser/showResults"); ?>';
                        } else {
